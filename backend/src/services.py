...
def get_all_chats(db: Session, user_uuid: uuid.UUID):
    try:
...
def create_chat(db: Session, chat: ChatCreate):
    try:
        ...
        for msg in chat.messages:
            db_message = Message(chat_uuid=db_chat.chat_uuid, content=msg.content, role=msg.role)
            db.add(db_message)
...
def update_chat(db: Session, chat: ChatUpdate):
    try:
        db_chat = db.query(Chat).filter(
            Chat.chat_uuid == chat.chat_uuid,
            Chat.user_uuid == chat.user_uuid
        ).first()
...
def delete_chat(db: Session, chat_uuid: uuid.UUID, user_uuid: uuid.UUID):
    try:
        db_chat = db.query(Chat).filter(Chat.chat_uuid == chat_uuid, Chat.user_uuid == user_uuid).first()
        if not db_chat:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chat not found")
